...
| 04:35 | Module I (额外修复) | **bug**: expand/rewrite/insert handler 在 LLM 调用前 flush 200 OK (streaming 优化), 但如果无 API key → ErrNoAPIKey 错误已无法改 status code (headers 已发) → client timeout. **修复**: 加 `checkAPIBeforeStream(w, r, provider, executor)` helper. executor == nil (test/mock) → 跳过. executor != nil → check ctx user key, 无 → 立即 503 (在 flush 200 之前). 加 3 个 call site (expand/rewrite/insert). review 不用 streaming flush, 直接调用 respondActionError 已足够. update 2 个 test cases | (bug fix) |
| 04:40 | Module I (终验) | deploy md5 9693c851 (fix 后). **smoke**: chapter expand 无 key → HTTP 503 `{"error":"user API key required: please configure your API key in SettingsPage (the server has no admin key fallback)"}`. 完美工作. CI #234 ✅ ALL GREEN | (verify) |
| 17:30 | Module J.1 | 启动 Module J (0.5d): 自动启动 + 启动时最小化到托盘. **新建包 `desktop/internal/settings/`**: `settings.go` (Settings struct + Load/Save/Default, JSON 持久化到 `%APPDATA%/novel2all-desktop/settings.json`, 原子写 tmp+rename) + `autostart_windows.go` (Registry HKCU\Software\Microsoft\Windows\CurrentVersion\Run\Novel2ALL, EnableAutoStart/DisableAutoStart/isAutoStartEnabled, 幂等) + `autostart_other.go` (macOS/Linux stub, `//go:build !windows`, 返 'auto-start not supported on this OS'). **desktop/app.go 加 3 个 wails-bound 方法**: GetSettings (返 *Settings, error, 加载失败返默认 + 错误) + UpdateSettings (s Settings, error, 持久化 settings.json + 调用 EnableAutoStart/DisableAutoStart, 单步失败返 error) + GetAutoStart (bool, error, 反射当前系统 Registry 实际状态). **main.go**: 启动时 settings.Load() 读 StartMinimized 设 wails.Run 的 StartHidden | (settings + desktop Go) |
| 17:35 | Module J.2 | **React UI**: SettingsPage 加 '桌面设置' section. **2 个 toggle**: '开机自启 (Windows Registry HKCU\...\Run)' + '启动时最小化到托盘 (不弹主窗口)'. **6 个新 state**: settings/settingsLoading/settingsSaving/settingsError/settingsSuccess/actualAutoStart. **useEffect 挂载时**: 并发拉 GetSettings (持久化) + GetAutoStart (实际 Registry). **saveSettings()**: 并发调 UpdateSettings + GetAutoStart 刷新实际状态. **同步显示 warning**: 持久化设置 vs 实际 Registry 状态不同步时 (用户从外部修改). **提示行**: 解释生效时机. **App.css 加 `.setting-toggle-row`** (flex + checkbox + label 样式) + `.settings-actions` (margin-top) | (React + CSS) |
| 17:45 | Module J.3 | **验证**: go vet ./... clean + golangci-lint v2.13.2 → 0 issues ✅ + desktop go test ./internal/... 14/14 secrets + 2/2 update PASS. **docs 加 3 条 Module J 条目**. **commit + push 待 CI**. **后端 deploy 不需要** (settings 仅桌面相关, 与后端 API 无关). smoke test: 用户在桌面 SettingsPage 启用自启 → 检查 Windows Registry HKCU\...\Run\Novel2ALL 注册; 禁用 → 删除 | (verify + docs) |
...
- [x] **Module J**: 自动启动 + 最小化到托盘 ✅ — `desktop/internal/settings/` 新包 (settings.go + autostart_windows.go + autostart_other.go build tag) + 3 个 wails 方法 (GetSettings/UpdateSettings/GetAutoStart) + SettingsPage 2 toggle UI + wails Run 的 StartHidden 选项. Registry 写 HKCU\Software\Microsoft\Windows\CurrentVersion\Run\Novel2ALL. macOS/Linux 是 stub (后续 Sprint 加 LaunchAgents/.desktop 文件)
